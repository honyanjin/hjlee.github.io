...
interface RecommendedPostsCarouselProps {
  posts: BlogPost[]
  categories: Category[]
...
const RecommendedPostsCarousel: React.FC<RecommendedPostsCarouselProps> = ({ posts, categories, getPostImageProps, getBackgroundStyle }) => {
        ...
        <div className="flex items-center justify-center gap-2 mt-6">
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              ...
              className={`h-2.5 w-2.5 rounded-full transition-colors ${i === currentPage ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`}
              aria-label={`추천 페이지 ${i + 1}`}
            />
          ))}
        </div>
      )}
...
export default RecommendedPostsCarousel
