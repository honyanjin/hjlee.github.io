    ...
    <PartnerLayout>
      <div className="px-4 max-w-5xl mx-auto">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 sm:p-6">
          ...
          {pages.length === 0 ? (
            <p className="text-gray-700 dark:text-gray-300">현재 할당된 파트너 페이지가 없습니다. 관리자에게 문의하세요.</p>
          ) : (
            <ul className="space-y-3">
              {pages.map(p => (
                <li key={p.id}>
                  <Link to={`/partner/pages/${p.id}`} className="text-blue-600 dark:text-blue-400 hover:underline">
                    {p.title}
                  </Link>
                </li>
              ))}
