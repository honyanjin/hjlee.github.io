...
const projectsSettingsSchema = z.object({
...
type ProjectsSettingsForm = z.infer<typeof projectsSettingsSchema>

const DEFAULT_SETTINGS: ProjectsSettingsForm = {
  hero_title: '',
  hero_description: '',
  hero_bg_image_url: '',
  hero_cta_label: '',
...
const AdminPagesProjects = () => {
  ...
  const [current, setCurrent] = useState<ProjectsPageSettingsT | null>(null)

  // Hero 설정 카드 접기/펼치기 상태 (로컬 스토리지에 저장)
  const [heroExpanded, setHeroExpanded] = useState<boolean>(() => {
    const saved = localStorage.getItem('projectsHeroExpanded')
  ...
  const {
    ...
    watch,
    formState: { errors }
  } = useForm<ProjectsSettingsForm>({
    resolver: zodResolver(projectsSettingsSchema),
  ...
  const toggleHeroExpanded = () => {
    ...
    localStorage.setItem('projectsHeroExpanded', JSON.stringify(newValue))
  }
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        ...
        if (data) {
          setCurrent(data as ProjectsPageSettingsT)
  ...
  const onSubmit = async (data: ProjectsSettingsForm) => {
    try {
      setIsSaving(true)
      setError('')
      setSuccess('')
      ...
      <div className="w-full">
        <header className="bg-white dark:bg-gray-800 shadow-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <Breadcrumb items={[{ label: '페이지 관리', path: '/admin/pages/projects' }, { label: 'Projects' }]} />
